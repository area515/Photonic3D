...
public class DispManXTest {
	...
	public interface DispManX extends Library {
		...
		public int vc_dispmanx_resource_create(int resourceType, int width, int height, byte[] imagePointer);
		public int vc_dispmanx_rect_set(VC_RECT_T.ByReference rectangleToCreate, int offsetX, int offsetY, int width, int height);
		public int vc_dispmanx_update_start(int iHaveNoIdea);//Constant of 10
		public int vc_dispmanx_element_add(
				int updateHandle, 
				int displayHandle,
				int iHaveNoIdea1,                            // 2000
				VC_RECT_T.ByReference screenRectangle, 
				int resourceHandle, 
				VC_RECT_T.ByReference blitRectangle,
		...
		public int vc_dispmanx_resource_write_data(
				...
				int pitch, 
				byte[] imageConvertedFromBufferedImage, 
	...
	public static void usage(String name ) {
		System.out.printf( "usage: %s <display> <duration in seconds> <file>\n", name );
	...
	public static byte[] loadBitmap(String fileName, int width, int height, IntByReference pitch) throws IOException {
		BufferedImage image = ImageIO.read(new File(fileName));
		byte[] destPixels = new byte[image.getWidth() * image.getHeight() * 3];
		int i = 0;
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		dispMan.bcm_host_init();
        if ( args.length < 2 ) {
        	usage( args[0] );
        }

        int screen = Integer.parseInt( args[1] );
        if ( args.length == 2 ) {
        	info(screen);
        }
        ...
        byte[] bitmap = loadBitmap( args[3], width.getValue(), height.getValue(), pitch );

        // create a texture
        int texture = dispMan.vc_dispmanx_resource_create( VC_IMAGE_TYPE_T.VC_IMAGE_RGB565.getcIndex(), width.getValue(), height.getValue(), bitmap );

        VC_RECT_T.ByReference copyRect = new VC_RECT_T.ByReference();
        VC_RECT_T.ByReference blitRect = new VC_RECT_T.ByReference();
        VC_RECT_T.ByReference screenRect = new VC_RECT_T.ByReference();
        res( "rect set", dispMan.vc_dispmanx_rect_set( copyRect, 0, 0, width.getValue(), height.getValue() ) );
        res( "rect set", dispMan.vc_dispmanx_rect_set( blitRect, 0, 0, width.getValue()<<16, height.getValue()<<16 ) );
        res( "rect set", dispMan.vc_dispmanx_rect_set( screenRect, 0, 0, width.getValue(), height.getValue() ) );

        int update = dispMan.vc_dispmanx_update_start( 10 );
        ...
        alpha.opacity = 255;
        int element = dispMan.vc_dispmanx_element_add( update, display, 2000, screenRect, texture, blitRect, DispManX.DISPMANX_PROTECTION_NONE, alpha, 0, VC_IMAGE_TRANSFORM_T.VC_IMAGE_ROT0.getcConst() );
        ...
        Thread.sleep( time );
        update = dispMan.vc_dispmanx_update_start( 10 );

        res( "element remove", dispMan.vc_dispmanx_element_remove( update, element ) );
        res( "submit", dispMan.vc_dispmanx_update_submit_sync( update ) );
        res( "resource delete", dispMan.vc_dispmanx_resource_delete( texture ) );
